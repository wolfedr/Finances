...
Public Class rptNPDetail
    ...
    Public strActivityDesc As String
    ...
    Private Sub rptDetail_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Dim n As Integer
        Dim strAccount As String = actID.Substring(0, 4)  ' "8305"
        ...
        strSql = "select (ltrim(str(a.jnlActNum)) + '-' + ltrim(str(a.jnlActSeq))) as Account,  "
        strSql += "sum(a.jnlTranAmount) as Total, jnlTranDate, b.actName, b.actDescription, c.busName, a.jnlID"
        strSql += " from tblDRJournal a left outer join tblBusiness c on  a.jnlTranBusinessID = c.BusID "
        strSql += " inner join tblDRCOA b on  (b.actNum = a.jnlActNum and a.jnlActSeq = b.actNumSeq) "
        strSql += " where b.actNum = " & strAccount
        strSql += " and jnlActivity = '" & strActivityDesc & "' "
        If Len(actID) > 4 Then
            Dim strSeq As String = actID.Substring(5, Len(actID) - 5)
            strSql += " and b.actNumSeq = " & strSeq
            strCaption = " " & strAccount & ": " & strSeq
        Else
            strCaption = strAccount
        End If
        strSql += " and a.jnlTranDate >= '" & strDetailStart & "' and a.jnlTranDate < '" & strDetailEnd & "' "
        strSql += " group by a.jnlActNum, a.jnlActSeq, a.jnlTranDate, b.actDescription, c.busName, b.actName, a.jnlID "
    ...
    Private Sub dgvActDetail_RowHeaderMouseClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles dgvActDetail.RowHeaderMouseClick
