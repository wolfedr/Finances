...
Public Class AllocateEntry
    ...
    Private Sub AllocateEntry_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        'ClearInput()
    ...
    Sub FIllATMEntries()
        ...
        strSql += "Where a.jnlTranBusinessID = b.busID and  a.jnlActNum = 8590 and jnlActSeq = 7 order by  a.jnlTranDate desc"
    ...
    Private Sub dgvAccounts_RowEnter(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvAccounts.RowEnter
        ...
        If LoadComplete = True Then
            If cell1.Value <> Nothing Then
                hdnAccount.Text = cell1.Value.ToString().Trim
        ...
        If btnAllocate.Text = "Allocate" Then
            ...
            If sngDivAmount > 0 Then 'If an amount is available to allocate
                If sngDivAmount < sngRemAmount Then 'If it is less than the amount availabe
                    ...
                    dgvLOA.Rows(n).Cells(7).Value = txtNewMemo.Text
                    dgvLOA.Rows(n).Cells(8).Value = ddlEntity.Text
                    If ddlEntity.SelectedValue = Nothing Then
                        dgvLOA.Rows(n).Cells(11).Value = 0
                        dgvLOA.Rows(n).Cells(8).Value = "Unlisted"
                    Else
                        dgvLOA.Rows(n).Cells(8).Value = ddlEntity.Text
                        dgvLOA.Rows(n).Cells(11).Value = ddlEntity.SelectedValue
                        '  dgvLOA.Rows(n).Cells(2).Value = RetrieveAccountFromBusID(ddlEntity.SelectedValue) 'moved to event
                    End If
                    hdnEntityID.Text = ddlEntity.SelectedValue
...
abnNote: MsgBox(strMsg, MsgBoxStyle.OkOnly)
    ...
    Private Sub ddlEntity_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles ddlEntity.SelectedIndexChanged
        If LoadComplete = True Then
            If ddlEntity.SelectedValue > 0 Then
                Dim n As Integer = hdnCounter.Text
                dgvLOA.Rows(n).Cells(2).Value = RetrieveAccountFromBusID(ddlEntity.SelectedValue)
                dgvLOA.Rows(n).Cells(8).Value = RetrieveBusNameFromBusID(ddlEntity.SelectedValue)
            End If
        End If
    End Sub
